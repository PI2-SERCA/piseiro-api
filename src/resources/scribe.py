...
class Scribe(Resource):
    def post(self):
        ...
        try:
            ...
            return jsonify(
                error="Bad Request: Wrong format in 'repetitions'",
                status=requests.codes.bad_request
            )
        ...
        result = []
        for cut, reps in cuts:
        	optimized_cut = get_optimized_cut(cut, ceramic) 
        	scribe_lines = get_scribe_lines(optimized_cut, ceramic)
        	#gcode = get_gcode(scribe_lines)
        	result.append({'cut': [list(optimized_cut.exterior.coords), reps]})
